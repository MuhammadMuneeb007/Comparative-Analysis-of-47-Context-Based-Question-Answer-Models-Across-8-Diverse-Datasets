...
df = pd.read_csv("Time_plot.csv",index_col="Models")
del df["Unnamed: 0"]
print(df.head())
...
del df["Model Size"]
del df["Overall Time in minutes"]
...
results.to_csv("Significance_plot_time.csv")
results = results.apply(pd.to_numeric)
plt.figure(figsize=(15, 15))
sns.heatmap(results, annot=True, cmap='coolwarm', fmt=".0f", linewidths=.5)

plt.title('Heatmap of p-values between Models execution time')
plt.tight_layout()
