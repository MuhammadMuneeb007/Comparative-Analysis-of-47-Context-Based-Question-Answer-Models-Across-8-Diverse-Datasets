...
columns  = []
for c in range(1,6):
    columns.append('AnswerLength_'+str(c)+'_Correct')
    columns.append('AnswerLength_'+str(c)+'_Wrong')
...
all_dataframes = pd.concat([frames[0],frames[1],frames[2],frames[3],frames[4],frames[5],frames[6],frames[7]])

results = sum(frames) / len(datasets)
results['models'] = pd.read_csv("answer_length_"+d+".csv")['models'].values
results = results.set_index('models')
results['Overall Performance'] = pd.read_csv("Accuracy_plot.csv")["Overall Performance"].values 
# Display the result
print(result)
