...
#data = pd.read_csv(loop+os.sep+loop+".csv",index_col=0)
#print(data.head())

d = 'JournalQA'

#dd = 
data = pd.read_csv(d+os.sep+d+".csv",encoding="iso-8859-1")
...
for c in sorted(data['type'].unique()):
    yes = []
    no = []
    
    for model in range(0,len(listofmodels)):
        temp = data[data['type']==c]
    ...
    resultss[str(c)+"_correct"] = yes
    resultss[str(c)+"_wrong"] = no

print(resultss.columns)
dic = {}
for c in listofmodels:
    dic[c] = []

for c in sorted(data['type'].unique()):

    max_row_index = resultss[c+"_correct"].idxmax()
    dic[resultss.loc[max_row_index]["Models"]].append(c)
...
plt.tight_layout() 
plt.savefig('plot11_'+d,dpi=1000)
